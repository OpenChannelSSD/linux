...
typedef struct nvm_addr *(nvm_lookup_ltop_fn)(struct nvm *, sector_t);
typedef struct nvm_addr *(nvm_map_ltop_page_fn)(struct nvm *, sector_t,
						int);
typedef struct nvm_block *(nvm_map_ltop_block_fn)(struct nvm *, sector_t,
						int);
typedef int (nvm_write_rq_fn)(struct nvm *, struct request *);
typedef int (nvm_read_rq_fn)(struct nvm *, struct request *);
typedef void (nvm_alloc_phys_addr_fn)(struct nvm *, struct nvm_block *);
typedef struct nvm_block *(nvm_lun_get_blk_fn)(struct nvm_lun *lun,
						int is_gc);
typedef void (nvm_lun_put_blk_fn)(struct nvm_block *block);
typedef int (nvm_ioctl_fn)(struct nvm *,
					unsigned int cmd, unsigned long arg);
typedef int (nvm_tgt_init_fn)(struct nvm *);
typedef void (nvm_tgt_exit_fn)(struct nvm *);
typedef void (nvm_endio_fn)(struct nvm *, struct request *,
...
typedef void (nvm_gc_kick_fn)(struct nvm *s);
typedef int (nvm_gc_init_fn)(struct nvm *s);
typedef void (nvm_gc_exit_fn)(struct nvm *s);
...
/* core.c */
struct request *nvm_inflight_laddr_acquire(struct nvm *s, sector_t,
						unsigned int, spinlock_t *);
void nvm_inflight_laddr_release(struct nvm *, struct request *);
...
/*   I/O request related */
int nvm_write_rq(struct nvm *, struct request *);
int nvm_read_rq(struct nvm *, struct request *rq);
int nvm_erase_block(struct nvm *, struct nvm_block *);
void nvm_update_map(struct nvm *, sector_t, struct nvm_addr *, int);
...
static inline struct nvm_ap *get_next_ap(struct nvm *s)
{
...
static inline struct nvm_lun *paddr_to_lun(struct nvm *s,
							sector_t p_addr)
{
	return &s->luns[p_addr / (s->nr_pages / s->nr_luns)];
}

static inline struct nvm_ap *block_to_ap(struct nvm *s,
							struct nvm_block *b)
{
...
static inline int physical_to_slot(struct nvm *s, sector_t phys)
{
...
static int __nvm_lock_laddr(struct nvm *s, sector_t laddr,
...
static inline int nvm_lock_laddr(struct nvm *s, sector_t laddr,
				 unsigned pages,
...
static inline int nvm_lock_rq(struct nvm *s, struct request *rq)
{
...
static inline void nvm_unlock_laddr(struct nvm *s, sector_t laddr,
				    struct nvm_inflight_rq *r)
{
	struct nvm_inflight *map =
			&s->inflight_map[laddr % NVM_INFLIGHT_PARTITIONS];
...
static inline void nvm_unlock_rq(struct nvm *s, struct request *rq)
{
	sector_t laddr = nvm_get_laddr(rq);
	unsigned int pages = blk_rq_bytes(rq) / EXPOSED_PAGE_SIZE;
	struct nvm_inflight_rq *r = nvm_get_inflight_rq(s->dev, rq);
