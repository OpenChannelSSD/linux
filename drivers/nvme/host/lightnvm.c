					   ...
					   unsigned int *nr_chunks)
{
	...
	*nr_chunks = min_t(int, ndev->geo.all_chunks, nr_zones);
	...
	if (nvme_nvm_get_chk_meta(ndev, chunks, chunk_off, *nr_chunks)) {
		kfree(chunks);
...
EXPORT_SYMBOL_GPL(nvme_nvm_zone_report);
...
void nvme_nvm_update_nvm_info(struct nvme_ns *ns)
{
	...
	int shift = ns->lba_shift - 9;
	int zone_size = dev->geo.addrf.sec_len + shift;

	geo->csecs = 1 << ns->lba_shift;
	geo->sos = ns->ms;

#ifdef CONFIG_BLK_DEV_ZONED
	blk_queue_chunk_sectors(ns->queue, 1 << zone_size);
	ns->queue->limits.zoned = BLK_ZONED_HM;
	ns->queue->nr_zones = get_capacity(ns->disk) >> zone_size;
#endif /* CONFIG_BLK_DEV_ZONED */
}
