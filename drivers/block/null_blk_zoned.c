...
{
	sector_t dev_size = (sector_t)dev->size * 1024 * 1024ULL;
	sector_t sector = 0;
	unsigned int i;

	dev->zone_size_sects = dev->zone_size << ZONE_SIZE_SHIFT;
	dev->nr_zones = dev_size >> (9 + ilog2(dev->zone_size_sects));
...
void null_zone_reset(struct nullb_cmd *cmd)
{
	...
	/*
	 * Note: clear or free the memory backing
	 * pages for the zones here !!!
	 */
}
